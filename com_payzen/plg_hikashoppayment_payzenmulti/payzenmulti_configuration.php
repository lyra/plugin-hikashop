...
$docs = '';
foreach (\Lyranetwork\Payzen\Sdk\Form\Api::getOnlineDocUri() as $lang => $docUri) {
    $label = $languages[$lang] ?? strtoupper((string) $lang);
    $docs .= '<a style="margin-left: 10px; text-decoration: none; text-transform: uppercase;" href="' . $docUri . 'hikashop/sitemap.html" target="_blank">' . $label . '</a>';
}
        ...
        <fieldset>
            ...
            <table>
                ...
                <tr>
                    ...
                    <td>
                        <a><b><?php echo \Lyranetwork\Payzen\Sdk\Form\Api::formatSupportEmails("https://payzen.io/fr-FR/support/", JText::_('PAYZENMULTI_CLICK_HERE')); ?></b></a>
                    </td>
                </tr>
            ...
            <table>
                ...
                <tr>
                    ...
                    <td>
                        ...
                        <img src="<?php echo HIKASHOP_LIVE . 'administrator' . DIRECTORY_SEPARATOR . 'components' . DIRECTORY_SEPARATOR . 'com_payzen' . DIRECTORY_SEPARATOR; ?>images/warn.png">
                ...
                <tr>
                    ...
                    <td>
                        <select name="data[payment][payment_params][payzenmulti_language]" class="inputbox" id="payzenmulti_language" style="width: 122px;" >
                            <?php
                            foreach (\Lyranetwork\Payzen\Sdk\Form\Api::getSupportedLanguages() as $code => $label) {
...
<tr>
    <td colspan="2">
        <fieldset>
            ...
            $deleteLabelJs = json_encode(JText::_('PAYZENMULTI_MULTI_OPTIONS_DELETE'));
            $js = 'var payzenDeleteLabel = ' . $deleteLabelJs . ';
                   function payzenAddOption(first, contract) {
                       if (first) {
                       ...
                       optionLine += "    <td><input name=\"data[payment][payment_params][payzen_multi_options][" + timestamp + "][count]\" style=\"width: 70px;\" type=\"text\" /></td>" +
                                     ...
                                     "    <td><button type=\"button\" onclick= \"payzenDeleteOption(" + timestamp + ");\">" + payzenDeleteLabel + " </td>" +
                                     "</tr>";
            ...
            <button id="payzen_multi_options_btn" type="button" style="<?php if (!empty($multi_options)) echo 'display: none;'; else echo ''; ?>"
                    onclick= "payzenAddOption(true, <?php echo $str_cb_avail; ?>);" >
    ...
    if (! empty($multi_options)) {
        foreach ($multi_options as $key => $option) {
            $label = $option['label'] ?? '';
            $amountMin = $option['amount_min'] ?? '';
            $amountMax = $option['amount_max'] ?? '';
            ...
            echo '<tr id="payzen_multi_option_' . $key . '">
                      <td>' . payzen_create_text('data[payment][payment_params][payzen_multi_options][' . $key . '][label]', $label, 'style="width: 150px;"') . '</td>
                      <td>' . payzen_create_text('data[payment][payment_params][payzen_multi_options][' . $key . '][amount_min]', $amountMin, 'style="width: 80px;"') . '</td>
                      <td>' . payzen_create_text('data[payment][payment_params][payzen_multi_options][' . $key . '][amount_max]', $amountMax, 'style="width: 80px;"') . '</td>';

            if ($cb_avail) {
                echo '<td>' . payzen_create_text('data[payment][payment_params][payzen_multi_options][' . $key . '][contract]', $contract, 'style="width: 70px;"') . '</td>';
            }

            echo '    <td>' . payzen_create_text('data[payment][payment_params][payzen_multi_options][' . $key . '][count]', $count, 'style="width: 70px;"') . '</td>
                      <td>' . payzen_create_text('data[payment][payment_params][payzen_multi_options][' . $key . '][period]', $period, 'style="width: 70px;"') . '</td>
                      <td>' . payzen_create_text( 'data[payment][payment_params][payzen_multi_options][' . $key . '][first]', $first, 'style="width: 70px;"') . '</td>
    ...
    function payzen_create_text($name, $value, $extra_attributes = '')
    {
        ...
        $output = '<input type="text" name="' . $safeName . '" value="' . $safeValue . '" ' . $extra_attributes . '>';
        return $output;
    }
    ...
    ?>
                            ...
                            <tr id="payzen_multi_option_add">
                                ...
                                <td>
                                    <button type="button" onclick="payzenAddOption(false, <?php echo $str_cb_avail; ?>);" >
                                        <?php echo JText::_('PAYZENMULTI_MULTI_OPTIONS_ADD'); ?>
                                    </button>
                                </td>
                            </tr>
        ...
        <fieldset>
            ...
            <table>
                <tr>
                    ...
                    <td>
                        <?php echo HTMLHelper::_('select.booleanlist', 'data[payment][payment_params][payzenmulti_redirect_enabled]' , '', @$params->payzenmulti_redirect_enabled); ?><br />
